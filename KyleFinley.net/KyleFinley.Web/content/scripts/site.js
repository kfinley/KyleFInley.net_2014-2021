...
function disableElement(element) {
    if ($("style#disabledElementStyles").length == 0) {
        $("<style id='disabledElementStyles'>.disabledElement{-ms-filter:'progid:DXImageTransform.Microsoft.Alpha(Opacity=30)';filter:alpha(opacity=30);-moz-opacity:0.3;-khtml-opacity:0.3;opacity:0.3;cursor:default;background-position:top;pointer-events:none;}</style>").appendTo("head");
    }
...
function handleAjaxFail(xhr, error, handleStatusOptions) {
    var settings = $.extend({}, {}, handleStatusOptions);
    var statusFuncs = {
        302: function (xhr, error) {
            try {
                location.href = xhr.responseJSON.Redirect;
            } catch (ex) {
                handleException("Exception: {0} xhr.status: {1}: error: {2}".format(ex, xhr.status, error), ex);
            }
        },
        400: function (xhr, error) {
            try {
                var runDefault = true;
                if (settings[400]) {
                    runDefault = settings[400](xhr, error);
                }
                if (runDefault) {
                    switch (xhr.responseJSON.Type) {
                        case "Validation":
                            var errors = "";
                            $(xhr.responseJSON.Errors).each(function (index, element) {
                                errors = errors + "\n" + element.Errors[0];
                                log(element.Field + " " + element.Errors[0]);
                            });
                            alert("Validation Errors!\n" + errors);
                            break;
                        case "Exception":
                            alert("Error: {0}".format(xhr.responseJSON.Exception));
                            break;
                        default:
                            alert("Unknown Type for AJAX BadRequest (status 400).");
                    }
                }
            } catch (ex) {
                handleException("Exception: {0} xhr.status: {1}: error: {2}".format(ex, xhr.status, error), ex);
        ...
        Default: function (xhr, error) {
            handleException("xhr.status: {0}: error: {1}".format(xhr.status, error));
        }
    };

    if (statusFuncs[xhr.status]) {
        statusFuncs[xhr.status](xhr, error);
    } else {
        statusFuncs.Default(xhr, error);
    }
};
...
function typeahead(element, remoreUrl, done, fail) {
    var target = new Bloodhound({
        datumTokenizer: Bloodhound.tokenizers.obj.whitespace("name"),
        queryTokenizer: Bloodhound.tokenizers.whitespace,
        remote: {
            url: remoreUrl + "/%QUERY",
            transport: function (settings, onDone, onFail) {
                $.ajax(settings.url)
            ...
            replace: function (url, query) {
                return url.replace('%QUERY', query.trim());
            }
        },
        limit: 10
    });
    ...
    },
    {
        ...
        log("Typeahead selected: {0} - {1}".format(datum.Name, datum.Id));
        $(this).data("id", datum.Id).data("name", datum.Name);
    });
};
